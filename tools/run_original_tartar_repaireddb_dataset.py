...
DATASET = (
    ROOT
    / "models"
    / "bound_modified_error_dataset_admissible_20260504_195052"
    / "TarTarRepairedDB"
    / "1repaireddb2"
)
OUT = ROOT / "experiments" / "original_tartar_repaireddb_20260506"
...
def run_job(mod_dir: Path, repair_kind: str) -> dict:
    ...
    cmd = (
        ...
        f"-run repaireddb_{repair_kind.lower()}_{mod_id} Job_RepairComputation "
        f"-Folder ../experiments/original_tartar_repaireddb_20260506/{repair_kind.lower()}/{mod_id}/Job_RepairComputation "
    ...
    printed_repairs = len(re.findall(r"^\s*Repair/", proc.stdout, flags=re.M))
    return {
        "id": mod_id,
        ...
        "printed_repairs": printed_repairs,
        "successful_repair": max(0, admissible) > 0 or printed_repairs > 0,
        "timed_out": proc.returncode == 124,
        "no_counterexample": "No trace file found" in proc.stdout,
        "qe_failed_or_exception": "Run QE failed" in proc.stdout or "Z3Exception" in proc.stdout or "Exception" in proc.stderr,
...
def main() -> int:
    ...
    successful_ids = sorted({r["id"] for r in results if r["successful_repair"]})
    summary = {
        "dataset": str(DATASET),
